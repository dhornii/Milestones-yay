...
# ==== area fungsi fk dan ik untuk kaki loky ====
# tranformasi: rotasi sumbu z dan translasi
def ztrans_3d(theta, x, y, z):
...
# transformasi: rotasi sumbu y dan translasi
def ytrans_3d(theta, x, y, z):
...
def FK_3dofspec_loky(th1, th2, th3, th_from_base):
...
def IK_3dofspec_loky(x_target, y_target, z_target, th_from_base):
    ...
    # sudut bagian femur
    x_0 = np.sqrt(x_target_new**2 + y_target_new**2)
    theta_f1 = np.arctan2(z_target_new, x_0 - 0.025)

    a_length = np.sqrt((x_0 - 0.025)**2 + z_target_new**2)
    arccos_in = (0.055**2 + a_length**2 - 0.071429**2) / (2 * a_length * 0.055)
    theta_f2 = np.arccos(arccos_in)
    theta_f = (theta_f1 + theta_f2)

    # sudut bagian tibia
    arccos_in_t =  (0.055**2 + 0.071429**2 - a_length**2) / (2 * 0.055 * 0.071429) 
    theta_t = (np.arccos(arccos_in_t) - np.deg2rad(90))
